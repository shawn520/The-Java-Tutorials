# The Java™ Tutorials （Java 学习教程中文笔记）
...
Java编程语言使用异常来处理错误和其他异常事件。 本课介绍何时以及如何使用异常。

## 什么是异常？

异常是执行程序期间发生的中断正常指令流程的事件。

术语“异常”是短语“异常事件”的缩写。

**定义**：异常是一个事件，它发生在程序执行过程中，扰乱了程序指令的正常流程。

当方法中发生错误时，该方法创建一个对象并将其传递给运行时系统。 该对象称为异常对象，其中包含有关错误的信息，包括错误发生时的类型和程序的状态。 创建一个异常对象并将其交给运行时系统称为抛出异常。

在一个方法抛出一个异常之后，运行时系统会尝试找到一些东西来处理它。 处理该异常的可能“一些事情”的集合是已调用的方法的有序列表，以获取发生错误的方法。 方法列表被称为调用栈（参见下图）。
...
本节涵盖了throw语句和Throwable类及其子类。

## The try-with-resources Statement

本节介绍try-with-resources语句，它是一个声明一个或多个资源的try语句。 资源是一个对象，在程序结束后必须关闭。 try-with-resources语句确保每个资源在语句结束时都关闭。

## 未经检查的异常 - 争议

本节解释了由RuntimeException的子类指示的未经检查的异常的正确和不正确的用法。

## 异常的优点

使用异常来管理错误与传统的错误管理技术相比具有一些优势。 您将在本节中了解更多信息。
